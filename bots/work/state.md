...
Updated: 2026-08-29 10:12 PM CT
...
- Memory (non-secret highlights):
  ...
  - Tax: gather expenses from bank and credit cards into two live reports (all expenses vs tax-deductible only). Do not guess deductibility. Ranch (Moondocks) expenses are the only business deductions besides normal personal ones. Lowe's and Tractor Supply accounts exist (~90% ranch). Both use email jeffgunson@hotmail.com plus a code Jeff enters (Lowe's text code; Tractor Supply sign-in-with-code emailed to that address). Do not store those passwords. Do not invent account numbers or a Tractor Supply username beyond what Jeff named. Sign-ins paused / not retrying until Jeff says to try again. Tax report as soon as both stores' purchase data are in; do not file unless he says yes to that exact filing. VeraBank history through 2025 was pulled for unfiled taxes (private on Work's computer; do not file; do not guess deductibility).
  ...
  - One-at-a-time todo loop (chat until A2P): current open setup items (no padding): 1) Area 9 phones (Jeff used next; keep that item) 2) bill names (learn from VeraBank / email; do not quiz) 3) medication whose list and names (Jeff used next; keep that item) 4) Lowe's then Tractor Supply purchase data (wait until Jeff says try again; then tax report; do not file unless yes) 5) jeff@gunson.us sign-in later 6) jeff@citizensratification.com sign-in later 7) iCloud sign-in once for Pictures and Files (do not start until Jeff says). Tracie email is done; do not connect. VeraBank sign-in is done; do not re-ask unless the session dies. No Gmail/Hotmail unless sessions break. No Electric. Next with no info keeps the current item. Credit stays open for more cards later (not a padded todo).
