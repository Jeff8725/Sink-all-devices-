...
- Memory (non-secret highlights):
  ...
  - One-at-a-time todo loop (chat until A2P): current open setup items (no padding): 1) Area 9 phones (Jeff used next; keep that item) 2) which bank (current) 3) bill names 4) medication whose list and names (Jeff used next; keep that item) 5) jeff@gunson.us sign-in later 6) jeff@citizensratification.com sign-in later 7) iCloud sign-in once for Pictures and Files (do not start until Jeff says). Tracie email is done; do not connect. No Gmail/Hotmail unless sessions break. No Electric. Next with no info keeps the current item. Credit stays open for more cards later (not a padded todo).
  - Remaining empty specialist shells: Church (church name now known as Emmanuel Baptist Church from theConnection; first-run still deferred). CRA is good-enough for now with the six files (never guess the rest). Gideon has Operations Manual plus theConnection scan of main areas (camp, churches, members, scripture numbers, officers); not every report or resource PDF was scanned. GideonNow and apparel store skipped. Family, Pictures, Files, Social Media, Credit, and Tax have been defined further since earlier snapshots.
